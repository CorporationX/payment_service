...
public class CurrencyConverterServiceImpl {
    ...
    public BigDecimal convertToTargetCurrency(String fromCurrency, BigDecimal amount) {
        ...
        if (fromRate == null || fromRate.compareTo(BigDecimal.ZERO) <= 0) {
            throw new CurrencyConversionException("Invalid exchange rate for " + fromCurrency);
        }

        if (targetRate == null || targetRate.compareTo(BigDecimal.ZERO) <= 0) {
            throw new CurrencyConversionException("Invalid exchange rate for " + targetCurrency);
        }

        BigDecimal amountInUsd = amount.divide(fromRate, DIVIDE_SCALE, RoundingMode.HALF_UP);
        BigDecimal amountInRub = amountInUsd.multiply(targetRate);
